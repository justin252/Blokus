...
let draw_panel panel_area =
  ...
  List.iteri (fun i piece ->
    ...
    let coords = (List.nth piece.Player.shape st.selected_orient).Player.coordinates in
...
let status_text () =
  ...
  in
  Printf.sprintf "%s | Piece %d/%d | Orient %d/%d [R to rotate]"
...
let () =
  ...
  let panel_w_widget = W.sdl_area ~w:panel_w ~h:panel_h () in
  ...
  let rotate_key _board =
    state := Gui_state.cycle_orient !state;
    refresh ()
  in
  ...
  let layout = L.tower [label_layout; game_row] in
  let r_shortcut = (Sdl.Kmod.none, Sdl.K.r, rotate_key) in
  let shortcuts = Main.shortcuts_of_list [Main.exit_on_escape; r_shortcut] in
